...
#val=("5fqd" "5hxb" "6h0f" "6h0g" "6uml" "7lps")
...
#echo -e "STRUCT_FILE\t${output_dir}merge/${iap}.mae\nJOB_TYPE\tREFINE\nPRIME_TYPE\tSIDE_PRED\nSELECT\t pick" > "${output_dir}prime_sidechain/header_ps_${iap}.inp"
...
chain_r=$(cat ${input_dir}${crbn}.pdb | awk '{print $5}' | head -n1)
...
#done > "${output_dir}prime_sidechain/bodyr_ps_${iap}.inp"
...
do
	...
	xyz_lig=$(echo "${xyz_lig}$(echo "$ligand_line" | awk '{print "["$2","$3","$4"]"}')")
...
do
	xyz_cer=$(echo "${xyz_cer}$(echo "$crbn_line" | awk '{print "["$2","$3","$4"]"}')")
...
pdb_id=$(echo "$iap" | cut -f1 -d "_")
loop="${input_dir}loops/${pdb_id}_target_loop.pdb"
...
#done > "${output_dir}prime_sidechain/bodyl_ps_${iap}.inp"
...
#echo -e "NUM_SC_OUTPUT_STRUCT\t1\nUSE_CRYSTAL_SYMMETRY\tno\nUSE_RANDOM_SEED\tno\nSEED\t0\nOPLS_VERSION\tS-OPLS\nEXT_DIEL\t80.00\nUSE_MEMBRANE\tno" > "${output_dir}prime_sidechain/end_ps_${iap}.inp"

echo -e "NUM_SC_OUTPUT_STRUCT\t1\nUSE_CRYSTAL_SYMMETRY\tno\nUSE_RANDOM_SEED\tno\nSEED\t0\nOPLS_VERSION\tS-OPLS\nEXT_DIEL\t80.00\nUSE_MEMBRANE\tno" > "${output_dir}prime_sidechain/end_ps_${iap}${crbn}.inp"


# Join all the lines in a single file
#cat "${output_dir}prime_sidechain/header_ps_${iap}.inp" "${output_dir}prime_sidechain/bodyr_ps_${iap}.inp" "${output_dir}prime_sidechain/bodyl_ps_${iap}.inp" "${output_dir}prime_sidechain/end_ps_${iap}.inp" > "${output_dir}prime_sidechain/ps_${iap}.inp"
...
#rm "${output_dir}prime_sidechain/header_ps_${iap}.inp"
#rm "${output_dir}prime_sidechain/bodyr_ps_${iap}.inp"
#rm "${output_dir}prime_sidechain/bodyl_ps_${iap}.inp"
#rm "${output_dir}prime_sidechain/end_ps_${iap}.inp"
